...
class PrepareRelease extends AbstractGradleStep {
    ...
    public String determineVersionDump() {
        ...
        buildContext.getScriptEngine() withCredentials([[$class: 'StringBinding', credentialsId: "${buildContext.getGroup()}-sonarqube-github-reporter", variable: 'GITHUB_OAUTH_TOKEN_NEW']]) {
            buildContext.getScriptEngine().sh "echo ${buildContext.getScriptEngine().env.GITHUB_OAUTH_TOKEN2}"
            buildContext.getScriptEngine().sh "curl -i -H \"Authorization: token ${buildContext.getScriptEngine().env.GITHUB_OAUTH_TOKEN_NEW}\" \$GITHUB_API_URL/repos/${buildContext.getGroup()}/${buildContext.getProject()}/issues/${prNumber}/labels | jq -r \".[].name\" > labels.txt"
